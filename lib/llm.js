...
/**
 * Probe the endpoint at startup with a minimal real request that exercises the
 * same URL, auth and model the app uses. Resolves to { ok: true, status, url,
 * model } on success, or { ok: false, error, ... } with diagnostic detail on
 * failure (HTTP status + response-body snippet, timeout, or network/DNS error).
 * Never throws — reachability is advisory; the app still runs with the fallback.
 */
async function checkHealth({ timeoutMs } = {}) {
  if (!isConfigured()) {
    const missing = ['LLM_BASE_URL', 'LLM_MODEL'].filter((k) => !process.env[k]);
    return { ok: false, configured: false, error: `not configured (missing ${missing.join(', ')})` };
  }
  const base = String(process.env.LLM_BASE_URL).replace(/\/+$/, '');
  const url = `${base}/chat/completions`;
  const model = process.env.LLM_MODEL;
  const key = apiKey();
  const t = num(timeoutMs, num(process.env.LLM_HEALTHCHECK_TIMEOUT_MS, 10000));
  const controller = new AbortController();
  ...
  try {
    ...
    if (!res.ok) {
      let body = '';
      try { body = (await res.text()).replace(/\s+/g, ' ').trim().slice(0, 300); } catch (_) { /* ignore */ }
      return {
        ok: false, status: res.status, url, model, hasKey: Boolean(key),
        error: `HTTP ${res.status} ${res.statusText}${body ? ` — ${body}` : ''}`,
      };
    }
    return { ok: true, status: res.status, url, model, hasKey: Boolean(key) };
  } catch (err) {
    let reason;
    if (err && err.name === 'AbortError') {
      reason = `timeout after ${t} ms`;
    } else {
      reason = (err && err.message) || String(err);
      // Node's fetch wraps low-level failures as "fetch failed"; the real code
      // (ECONNREFUSED, ENOTFOUND, certificate errors, …) lives on err.cause.
      const cause = err && err.cause && (err.cause.code || err.cause.message);
      if (cause) reason += `: ${cause}`;
    }
    return { ok: false, url, model, hasKey: Boolean(key), error: reason };
  } finally {
    clearTimeout(timer);
  }
}

module.exports = { complete, checkHealth, isConfigured, FALLBACK_MESSAGE };
